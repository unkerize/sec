...
//  - STM32F103
//------------------------------------------------------------------
#define spiTxDmaBlockMask (DMA_CCR1_MINC |  DMA_CCR1_PSIZE_1 | DMA_CCR1_DIR  | DMA_CCR1_EN)
#define spiTxDmaDummyMask (            0 |  DMA_CCR1_PSIZE_1 | DMA_CCR1_DIR  | DMA_CCR1_EN)
#define spiRxDmaBlockMask (DMA_CCR1_MINC |  DMA_CCR1_PSIZE_1 | DMA_CCR1_TCIE | DMA_CCR1_EN)
#define spiRxDmaDummyMask (            0 |  DMA_CCR1_PSIZE_1 | DMA_CCR1_TCIE | DMA_CCR1_EN)
//------------------------------------------------------------------
// common drv code
#include "spi_drv.c"
//------------------------------------------------------------------
...
uint8_t spiAdd(SPI_TypeDef* lspi_port, uint32_t lspi_options)
{
  ...
  spi[spiCounter].port->CR2 = SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN;
  ...
  spi[spiCounter].txDma->CCR = DMA_CCR1_PL_0; // mark dma
  spi[spiCounter].rxDma->CCR = DMA_CCR1_PL_0; // mark dma
...
uint8_t spiTxByte(uint8_t spih, uint8_t txValue)
{
  ...
  spi[spih].port->DR = txValue;
  while ( (spi[spih].port->SR & SPI_SR_RXNE) != SPI_SR_RXNE ) {};
  return spi[spih].port->DR;
}
//------------------------------------------------------------------
//------------------------------------------------------------------
