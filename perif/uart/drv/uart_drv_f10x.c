...
#define uartMaxNumber 5

#define uartTxDmaBlockMask (DMA_CCR1_MINC |  DMA_CCR1_PSIZE_1 | DMA_CCR1_DIR  | DMA_CCR1_EN)
#define uartRxDmaBlockMask (DMA_CCR1_MINC |  DMA_CCR1_PSIZE_1 | 0             | DMA_CCR1_EN)
...
void uartSetBaudeRate(uint8_t luart, uint32_t lBaudeRate)
{
  ...
  uart[luart].port->BRR = (uartBaudDiv/lBaudeRate) & 0x0000fff0;
}
...
uint8_t uartAdd(uartCfgType* lCfg)
{
  ...
  if (uart[uartCounter].port == USART2)
  {
    ...
    uart[uartCounter].txDMA = DMA1_Channel7;
    uart[uartCounter].rxDMA = DMA1_Channel6;
  ...
  if (uart[uartCounter].port == USART3)
  {
    uart_handler[2] = uartCounter;
    uartIRQ     = USART3_IRQn;

    uart[uartCounter].txDMA = DMA1_Channel2;
    uart[uartCounter].rxDMA = DMA1_Channel3;
  ...
  if (uart[uartCounter].port == UART4)
  {
    uart_handler[3] = uartCounter;
    uartIRQ     = UART4_IRQn;

    uart[uartCounter].txDMA = DMA2_Channel5;
    uart[uartCounter].rxDMA = DMA2_Channel3;
  ...
  if (uart[uartCounter].port == UART5)
  {
    uart_handler[4] = uartCounter;
    uartIRQ     = UART5_IRQn;

    uart[uartCounter].txDMA = DMA2_Channel1;
    uart[uartCounter].rxDMA = DMA2_Channel4;
  ...
  uart[uartCounter].port->BRR = (uartBaudDiv/lCfg->BaudeRate) & 0x0000fff0;
  uart[uartCounter].port->CR3 = USART_CR3_DMAT | USART_CR3_DMAR;

  uart[uartCounter].port->SR;
  uart[uartCounter].port->DR;
  uart[uartCounter].port->SR = 0;
  ...
  uart[uartCounter].txDMA->CCR = DMA_CCR1_PL_0; // mark dma
  uart[uartCounter].rxDMA->CCR = DMA_CCR1_PL_0; // mark dma

  uart[uartCounter].txDMA->CPAR = (uint32_t) &uart[uartCounter].port->DR;
  uart[uartCounter].rxDMA->CPAR = (uint32_t) &uart[uartCounter].port->DR;
...
{
  if (uart[luart].port->SR & USART_SR_IDLE)
  {
    uartRxStop(luart);
  ...
  if (uart[luart].port->SR & USART_SR_TC)
  {
    uart[luart].port->SR &= ~USART_SR_TC;
...
{
  uart_irq_handler(uart_handler[2]);
}

#ifdef RCC_APB1ENR_UART4EN
void UART4_IRQHandler(void)
{
  uart_irq_handler(uart_handler[3]);
}
#endif
...
{
  uart_irq_handler(uart_handler[4]);
}
#endif
