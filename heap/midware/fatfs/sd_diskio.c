...
//  uint32_t counter = 0;
  ...
  if (SD_IO_WriteCmd(SD_CMD_SEND_CSD, 0, 0xFF, SD_RESPONSE_NO_ERROR) == 0)
  {
    if (SD_IO_WaitResponse(SD_START_DATA_SINGLE_BLOCK_READ) == 0)
    {
      ...
      spiTxBlock(spih, 0, CSD_Tab, 16+2); // +2 dummy-CRC (buffer increased!)
...
//  uint32_t counter = 0;
  ...
  if (SD_IO_WriteCmd(SD_CMD_SEND_CID, 0, 0xFF, SD_RESPONSE_NO_ERROR) == 0)
  {
    if (SD_IO_WaitResponse(SD_START_DATA_SINGLE_BLOCK_READ) == 0)
    {
      ...
      spiTxBlock(spih, 0, CID_Tab, 16+2); // +2 dumm (CRC) // buf increased
...
DSTATUS SD_initialize(void)
{
  ...
  // SD SPI Config
  spih =  spiAdd(SPI2, spioMODE3 | spioMSBF | _SD_SPI_BR | spiOSStyle | (spirtsh<<spioOSindx_bp) );
  spiSetDummy(spih, 0xff);
  ...
  if (SD_GoIdleState() != MSD_OK) return Stat;
